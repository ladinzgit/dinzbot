...
class MusicConfigView(discord.ui.View):
    ...
    def build_embed(guild: discord.Guild) -> discord.Embed:
        ...
        embed = discord.Embed(
            title='🎵 음악봇 설정',
            description='음악 채널을 지정하면 해당 채널에 고정 플레이어가 생성됩니다.',
            color=0x1db954,
        )
        ...
        embed.add_field(name='🎙️ 음악 채널', value=ch_text, inline=True)
        ...
        embed.set_footer(text='아래 버튼으로 설정을 변경하세요  •  관리자 전용')
        return embed

    @discord.ui.button(label='채널 설정', emoji='🎙️', style=discord.ButtonStyle.primary, row=0)
